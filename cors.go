...
func (r *Router) Cors(opts Options) Handler {
	...
	return func(res http.ResponseWriter, req *http.Request, p Params) {
		...
		if req.Method == "OPTIONS" {
			...
			headers := req.Header.Get(headerRequestHeaders)
			opts.setPreflightHeaders(origin, method, headers, req.Response.Header)
			r.Abort()
			return
		}
		opts.setActualHeaders(origin, req.Response.Header)
		return
	}
}
